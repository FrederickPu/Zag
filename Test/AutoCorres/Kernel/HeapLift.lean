...
def readEvidence :
    Zag.Lang.AutoCorres.HeapLift.Kernel.ExprEvidence stateMap readConcrete where
  ...
  rewrite := by simp [Zag.Lang.AutoCorres.HeapLift.struct_rewrite_expr]
  rewriteGuardAbstracts := by simp [Zag.Lang.AutoCorres.HeapLift.abs_guard]
  expressionAbstracts := by
    simp [Zag.Lang.AutoCorres.HeapLift.abs_expr, stateMap, readConcrete,
      readAbstract]

def writeEvidence (value : Nat) :
    Zag.Lang.AutoCorres.HeapLift.Kernel.UpdateEvidence stateMap
      (writeConcrete value) where
  rewritePrecondition := fun _ => True
  rewritten := writeConcrete value
  abstractRewriteGuard := fun _ => True
  abstractUpdateGuard := isValid
  abstract := writeAbstract value
  rewrite := by simp [Zag.Lang.AutoCorres.HeapLift.struct_rewrite_modifies]
  rewriteGuardAbstracts := by simp [Zag.Lang.AutoCorres.HeapLift.abs_guard]
  updateAbstracts := by
    simp [Zag.Lang.AutoCorres.HeapLift.abs_modifies, stateMap, writeConcrete,
      writeAbstract]

def source :
    Zag.Lang.AutoCorres.HeapLift.Kernel.Source ConcreteState Unit Unit :=
...
theorem source_alias_has_canonical_l2_denotation :
    Zag.Lang.AutoCorres.HeapLift.Kernel.Source.denote source =
      L2.Syntax.denote (source : L2.Syntax ConcreteState Unit Unit) := by
  rfl

def supported :
    Zag.Lang.AutoCorres.HeapLift.Kernel.Supported stateMap source :=
...
theorem exact_target :
    certificate.target =
      Zag.Lang.AutoCorres.HeapLift.Kernel.Target.seq
...
theorem read_guard_accepts_valid :
    ...
    | _ => False := by
  simp [certificate, supported, ML.HeapLift.transform, readEvidence, isValid]
...
theorem certified :
    Zag.Lang.AutoCorres.HeapLift.L2Tcorres stateMap
      certificate.target.denote source.denote :=
...
theorem target_runs_when_valid :
  ...
  simp [certificate, supported, ML.HeapLift.transform,
    Zag.Lang.AutoCorres.HeapLift.Kernel.Target.denote, readEvidence,
    writeEvidence, readAbstract, writeAbstract, isValid, L2.seq, L2.guard,
    L2.gets, L2.modify]
...
theorem valid_target_does_not_fail :
  ...
  simp [certificate, supported, ML.HeapLift.transform,
    Zag.Lang.AutoCorres.HeapLift.Kernel.Target.denote, readEvidence,
    writeEvidence, isValid, L2.seq, L2.guard, bindE, L2.failed_liftE,
...
theorem target_fails_when_invalid :
  ...
  simp [certificate, supported, ML.HeapLift.transform,
    Zag.Lang.AutoCorres.HeapLift.Kernel.Target.denote, readEvidence, isValid,
    L2.seq, L2.guard, bindE, L2.failed_liftE, Zag.Lang.AutoCorres.guard]
...
theorem invalid_target_has_no_results
  ...
  simp [Zag.Lang.AutoCorres.HeapLift.Kernel.Target.denote, isValid,
    L2.seq, L2.guard, bindE, Zag.Lang.AutoCorres.bind,
